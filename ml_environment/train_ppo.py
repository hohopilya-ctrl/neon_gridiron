...
class MultiAgentToVecEnv(VecEnv):
    ...
    def __init__(self, env):
        ...
        super().__init__(
            self.num_agents, env.observation_space["agent_0"], env.action_space["agent_0"]
        )
    ...
    def step_wait(self):
        ...
        dones_array = np.array(
            [term_dict[f"agent_{i}"] or trunc_dict[f"agent_{i}"] for i in range(self.num_agents)]
        )
...
def train():
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=50000, save_path="./models/", name_prefix="neon_gridiron_ppo"
    )
