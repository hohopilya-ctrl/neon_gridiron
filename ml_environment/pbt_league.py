...
class CompetitiveSelfPlayVecEnv(VecEnv):
    def __init__(self, opponent_model_path=None, learning_team=0):
        ...
        super().__init__(
            self.num_agents, self.env.observation_space["agent_0"], self.env.action_space["agent_0"]
        )
        self.last_obs = None
    ...
    def step_wait(self):
        ...
        if self.opponent_model is not None:
            ...
            for global_i in opp_idx:
                action_dict[f"agent_{global_i}"] = self.env.action_space[
                    f"agent_{global_i}"
                ].sample()
        ...
        dones_array = np.array(
            [term_dict[f"agent_{i}"] or trunc_dict[f"agent_{i}"] for i in learning_idx]
        )
    ...
    for i in range(num_models):
        ...
        if os.path.exists(target_path):
            ...
            if base_model_path and os.path.exists(base_model_path):
                ...
                policy_kwargs = dict(
                    net_arch=dict(pi=[512, 512, 256], qf=[512, 512, 256], vf=[512, 512, 256])
                )
        ...
        population.append(
            {"name": model_name, "path": model_path, "elo": 1200, "lr": model.learning_rate}
        )
...
def play_match(model_a_info, model_b_info, timesteps=30000):
    ...
    print(
        f"Training {model_a_info['name']} against {model_b_info['name']} for {timesteps} steps..."
    )
